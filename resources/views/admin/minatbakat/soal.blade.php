...
<body class="bg-[#E9EFFF] min-h-screen p-3 sm:p-6 md:p-8">
    ...
    <div class="max-w-[1440px] mx-auto" x-data="soalApp()">

        <div class="mb-6">
            <button onclick="window.history.back()"
                class="group flex items-center gap-2 text-blue-500 font-bold text-xs uppercase tracking-widest hover:text-blue-700">
                <div
                    class="w-10 h-10 bg-white rounded-xl shadow-sm flex items-center justify-center group-hover:bg-blue-500 group-hover:text-white transition-all">
        ...
        <div x-show="showFormAdd" x-cloak class="mb-8">
            <div class="bg-white p-6 rounded-3xl shadow-xl border-b-8 border-blue-400">
                <textarea x-model="newText"
                    class="w-full p-4 bg-blue-50/50 border-none rounded-2xl focus:ring-2 focus:ring-blue-400 outline-none mb-4"
                    placeholder="Tulis soal baru..." rows="3"></textarea>
                <div class="flex justify-end gap-3">
                    <button @click="showFormAdd = false" class="text-xs font-bold text-blue-400 px-4">Batal</button>
                    <button @click="saveNew()"
                        class="px-8 py-3 bg-[#4A72D4] text-white rounded-xl text-xs font-bold shadow-lg">Simpan</button>
                </div>
            </div>
        </div>
        ...
        <div class="space-y-4 min-h-[300px]">
            <template x-for="(soal, index) in paginatedQuestions" :key="soal.id">
                <div
                    class="group bg-white p-6 rounded-[2.5rem] shadow-sm border border-transparent hover:border-blue-200 flex items-start gap-4">
                    ...
                    <button @click="deleteSoal(soal)"
                        class="w-12 h-12 bg-red-50 text-red-400 rounded-2xl hover:bg-red-500 hover:text-white transition-all flex items-center justify-center opacity-0 group-hover:opacity-100">
        ...
        <div class="mt-10 flex items-center justify-center gap-2" x-show="totalPages > 1">
            <button @click="currentPage--" :disabled="currentPage === 1"
                class="w-10 h-10 rounded-xl bg-white disabled:opacity-20 shadow-sm"><i
                    class="fa-solid fa-chevron-left"></i></button>
            <div class="flex gap-2">
                <template x-for="p in totalPages" :key="p">
                    <button @click="currentPage = p" class="w-10 h-10 rounded-xl bg-white font-bold text-xs shadow-sm"
        ...
        <div x-show="showHistory" x-cloak class="fixed inset-0 z-[100] flex items-center justify-end">
            ...
            <div x-show="showHistory" x-transition:enter="transition transform duration-300"
                ...
                <div class="p-6 bg-[#4A72D4] text-white flex items-center justify-between">
                    ...
                    <button @click="showHistory = false" class="w-10 h-10 rounded-full bg-white/20 hover:bg-white/40"><i
                    ...
                    <template x-for="h in history" :key="h.id">
                        <div class="bg-white rounded-3xl p-5 shadow-sm border border-blue-100">
                            ...
                            <div class="flex justify-between items-center border-t pt-3">
                                <span class="text-[9px] font-bold text-blue-300 uppercase"
                                    x-text="h.deletedAt"></span>
                                <div class="flex gap-2">
                                    <button @click="restoreSoal(h)"
                                        class="px-3 py-1 bg-emerald-100 text-emerald-600 rounded-lg font-bold text-[9px]">PULIHKAN</button>
                                    <button @click="forceDelete(h.id)"
                                        class="w-7 h-7 bg-red-100 text-red-400 rounded-lg flex items-center justify-center"><i
