...
<body class="bg-[#E9EFFF] h-screen flex overflow-hidden text-[#2D3B61]" x-data="{
    ...
    <main class="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
         <header class="flex flex-col lg:flex-row lg:items-center justify-between p-4 lg:px-8 lg:pt-8 lg:pb-4 gap-4 flex-shrink-0 w-full mb-8">
    ...
    <div class="flex items-center justify-between w-full lg:w-auto gap-4 lg:order-2">
        ...
        <div x-data="{ open: false }" class="relative flex-1 lg:flex-initial">
            <div @click="open = !open" 
                class="flex items-center justify-between lg:justify-start gap-3 bg-white p-1 pr-4 pl-1 rounded-full shadow-sm cursor-pointer border border-transparent hover:border-blue-100 transition-all w-full lg:min-w-[160px]">
    ...
    >
        ...
        {{ $item->is_active ? 'hover:bg-emerald-100 text-emerald-600' : 'hover:bg-gray-100 text-gray-500' }}">
                            ...
                            @click="
                                Swal.fire({
                            title: 'Hapus Latihan?',
                            text: 'Latihan akan dipindahkan ke History',
                            icon: 'warning',
                            width: '340px',
                    ...
                    <form action="{{ route('admin.latihan.destroy', $item->id) }}" method="POST" class="hidden">
                        @csrf
                        @method('DELETE')
                    </form>
                    ...
                    <table x-show="activeTab === 'history'"
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            @click="
                                Swal.fire({
                            title: 'Pulihkan Latihan?',
                            text: 'Data akan dikembalikan ke daftar Latihan',
                            icon: 'question',
                            width: '340px',
                            showCancelButton: true,
                    ...
                    <form action="{{ route('admin.latihan.restore', $trashItem->id) }}" method="POST" class="hidden">
                        @csrf
                    </form>
                    ...
                    <form action="{{ route('admin.latihan.forceDelete', $trashItem->id) }}" method="POST" class="hidden">
                        @csrf
                        @method('DELETE')
                    </form>
