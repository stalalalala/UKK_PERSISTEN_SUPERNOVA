    ...
    <style>
        ...
        @media (min-width: 1024px) {
            ...
            .admin-layout {
                ...
                height: 100vh;
                overflow: hidden;
                /* Pastikan layout utama tidak ikut scroll */
            }
        }
    </style>
</head>
...
<body class="bg-[#F8FAFC]" x-data="monitoringApp()">
    ...
    <div class="admin-layout">
        <aside x-data="{ currentPage: 'laporan' }" :class="mobileMenuOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'"
            ...
            <nav
                class="flex-1 px-6 space-y-1 overflow-y-auto pr-4 
            ...
            <div class="p-6 pt-2">
                <form action="{{ route('logout') }}" method="POST" class="w-full inline">
                    @csrf
                    <button type="submit"
                        class="w-full flex items-center bg-white/10 hover:bg-white/20 px-6 py-3 rounded-2xl transition-all group border border-white/20 backdrop-blur-sm shrink-0">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2"
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div class="bg-white rounded-[2rem] border border-slate-200 shadow-sm flex flex-col h-[400px]">
                        <div class="p-6 border-b">
                            <h3 class="text-[10px] font-black uppercase text-slate-500 tracking-widest">Aktivitas Saya
                            </h3>
                        </div>
                        <div class="flex-1 overflow-y-auto log-scroll p-4 space-y-4">
                            <template x-for="log in myLogs" :key="'my-' + log.id">
                                <div
                                    class="p-5 bg-white border border-slate-100 rounded-3xl shadow-sm hover:border-[#4A72D4] transition-all">
                                    <div class="flex justify-between items-start mb-2">
                                        <div class="flex flex-col gap-1">
                                            <span
                                                class="text-[9px] font-black px-2 py-1 bg-[#4A72D4] text-white rounded-lg uppercase w-fit"
                                                x-text="log.aksi"></span>
                                            <span class="text-[9px] font-bold text-[#4A72D4] italic pl-1"
                                                x-text="log.hari"></span>
                                        </div>
                    ...
                    <div class="bg-white rounded-[2rem] border border-slate-200 shadow-sm flex flex-col h-[400px]">
                        <div class="p-6 border-b">
                            <h3 class="text-[10px] font-black uppercase text-slate-500 tracking-widest">Riwayat Semua
                                Admin</h3>
                        </div>
                        <div class="flex-1 overflow-y-auto log-scroll p-4 space-y-3">
                            <template x-for="log in filteredLogs" :key="'all-' + log.id">
                                <div
                                    class="p-4 bg-slate-50 rounded-[1.5rem] border border-transparent hover:border-slate-200 transition-all">
                                        ...
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <p class="text-[11px] font-black text-slate-700"
                                                    x-text="'Admin: ' + (log.nama_admin || 'Unknown')"></p>
                                                <p class="text-[9px] text-[#4A72D4] font-bold uppercase"
                                                    x-text="log.aksi"></p>
                                            </div>
                                        </div>
                ...
                <div class="space-y-6 pb-20">
                    <div class="flex items-center gap-6 border-b border-slate-100 pb-4">
                        <button @click="activeTab = 'archive'"
                            :class="activeTab === 'archive' ? 'text-slate-800 border-[#4A72D4]' :
                                'text-slate-400 border-transparent'"
                            class="flex items-center gap-2 pb-2 border-b-2 transition-all">
                        ...
                        <button @click="activeTab = 'history'"
                            :class="activeTab === 'history' ? 'text-slate-800 border-[#4A72D4]' :
                                'text-slate-400 border-transparent'"
                            class="flex items-center gap-2 pb-2 border-b-2 transition-all">
                    ...
                    <div x-show="activeTab === 'archive'" x-transition>
                        <template x-for="item in archiveData" :key="item.tahun">
                            <div
                                class="bg-white rounded-[2rem] border border-slate-100 overflow-hidden shadow-sm mb-4">
                                <button @click="openYear = (openYear === item.tahun ? null : item.tahun)"
                                    class="w-full flex items-center justify-between p-6 hover:bg-slate-50">
                                    <span class="text-lg font-black text-slate-800" x-text="item.tahun"></span>
                                    <svg class="w-5 h-5 transition-transform"
                                        :class="openYear === item.tahun ? 'rotate-180' : ''" fill="none"
                                        stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-width="3" d="M19 9l-7 7-7-7" />
                                    </svg>
                                </button>
                                <div x-show="openYear === item.tahun" x-collapse
                                    class="p-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 bg-slate-50/30">
                                    <template x-for="bln in item.bulan" :key="bln.nama">
                                        <div class="bg-white p-2 rounded-3xl border border-slate-100">
                                            <button @click="openMonth = (openMonth === bln.nama ? null : bln.nama)"
                                                class="w-full p-4 flex items-center gap-3 hover:bg-amber-50 rounded-2xl text-left">
                                                <svg class="w-5 h-5 text-amber-400 shrink-0" fill="currentColor"
                                                    viewBox="0 0 20 20">
                                                    <path
                                                ...
                                                <span class="text-[11px] font-black text-slate-700 uppercase"
                                                    x-text="bln.nama"></span>
                                            </button>
                                            <div x-show="openMonth === bln.nama" x-collapse class="space-y-1 p-2">
                                                <template x-for="mgg in bln.minggu">
                                                    <div
                                                        class="group flex items-center justify-between p-1 hover:bg-blue-50 rounded-xl">
                                                        <button @click="openDraft(item.tahun, bln.nama, mgg)"
                                                            class="flex-1 text-left p-2 pl-4 text-[10px] font-bold text-slate-400 group-hover:text-[#4A72D4]">
                                            ...
                                            <div class="space-y-3">
                                                <template x-for="mgg in bln.minggu">
                                                    <div class="bg-white p-3 rounded-2xl shadow-sm">
                                                        <p class="text-[10px] font-black text-slate-800 mb-3"
                                                            x-text="'Minggu ' + mgg"></p>
    ...
    <div x-show="showDraftDetail" x-transition:enter="transition ease-out duration-300"
        ...
        <div @click.away="showDraftDetail = false"
            ...
            <div class="bg-slate-900 p-5 sm:p-8 text-white flex justify-between items-start gap-4">
                <div class="min-w-0">
                    <h4 class="text-lg sm:text-xl lg:text-2xl font-black uppercase tracking-tight truncate"
                        x-text="selectedDraftTitle"></h4>
                    <p class="text-slate-400 text-[9px] sm:text-[10px] uppercase tracking-[0.2em] font-bold mt-1">
                        Laporan Aktivitas Mingguan</p>
                    ...
                    <button
                        ...
                        <i
                            class="fa-solid fa-file-pdf text-xs sm:text-sm group-hover:rotate-12 transition-transform"></i>
            ...
            <div class="p-4 sm:p-8 overflow-y-auto log-scroll bg-slate-50/50 flex-1">
                <div class="space-y-4 sm:space-y-1">
                    <template x-for="log in filteredLogsByDraft">
                        <div
                            class="flex flex-col sm:flex-row gap-3 sm:gap-6 pb-6 border-b border-slate-100 mb-6 last:border-0 last:mb-0 last:pb-0 group">

                            <div
                                class="flex sm:flex-col justify-between sm:justify-start items-center sm:items-end sm:min-w-[100px] bg-white sm:bg-transparent p-2 sm:p-0 rounded-lg border sm:border-0 border-slate-100 shadow-sm sm:shadow-none">
                                <p class="text-[10px] font-black text-[#4A72D4] uppercase" x-text="log.hari"></p>
                                <p class="text-[9px] font-bold text-slate-400 sm:text-slate-300 italic group-hover:text-slate-400 transition-colors"
                                    x-text="log.jam"></p>
                            </div>
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-2">
                                    ...
                                    <p class="text-[10px] sm:text-[11px] font-black text-slate-500 uppercase tracking-wider truncate"
                                        x-text="'Admin: ' + log.nama_admin"></p>
                                </div>
                                <p class="text-xs sm:text-sm font-black text-slate-800 mt-1 sm:mt-1.5 leading-relaxed"
                                    x-text="log.objek"></p>

                                <div
                                    class="mt-2 bg-white p-3 sm:p-4 rounded-xl border border-slate-100 shadow-sm group-hover:border-blue-100 transition-colors">
                                    <p class="text-[10px] sm:text-[11px] text-slate-500 italic leading-relaxed"
                                        x-text="'» ' + log.detail"></p>
                                </div>
                            </div>
                        </div>
                ...
                <template x-if="filteredLogsByDraft.length === 0">
                    <div class="py-12 sm:py-20 text-center">
                        <div
                            class="inline-flex items-center justify-center w-16 h-16 sm:w-20 sm:h-20 bg-slate-100 rounded-full mb-4">
                            <i class="fa-solid fa-folder-open text-3xl sm:text-4xl text-slate-300"></i>
                        </div>
                        <p class="text-slate-400 text-[10px] sm:text-xs font-bold uppercase tracking-widest px-4">Tidak
                            ada aktivitas pada draf ini</p>
                    </div>
                </template>
            </div>
            ...
            <div class="px-5 sm:px-8 py-3 sm:py-4 bg-white border-t border-slate-50 flex justify-between items-center">
                ...
                <p class="text-[8px] sm:text-[9px] font-bold text-slate-300 uppercase tracking-[0.2em]">Persisten
                    Monitoring System</p>
            </div>
        </div>
    </div>
