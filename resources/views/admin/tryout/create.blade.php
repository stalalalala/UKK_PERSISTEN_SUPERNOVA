            ...
            <form id="formTryout" action="{{ route('admin.tryout.store') }}" method="POST">
                ...
                <div x-show="activeSubtesIndex === null" x-transition>
                    <div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
                        ...
                        <div class="flex flex-wrap gap-2 w-full md:w-auto">
                            <button type="button" @click="showImportModal = true"
                                class="w-full md:w-auto bg-green-500 hover:bg-green-600 text-white border-blue-100 px-6 py-3 rounded-2xl font-bold text-xs shadow-lg flex items-center justify-center gap-2 hover:scale-105 transition-all">
                    ...
                    <div
                        ...
                        <div class="space-y-2">
                            <label class="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Nama
                                Tryout *</label>
                            <input type="text" x-model="namaTryout" @input="saveLocal()" @input="isDirty = true"
                                required class="w-full bg-gray-50 rounded-2xl py-4 px-6 text-sm outline-none">
                        </div>
                        <div class="space-y-2">
                            <label class="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Mulai
                                *</label>
                            <input type="date" x-model="tglMulai" @input="saveLocal()" @input="isDirty = true"
                                required class="w-full bg-gray-50 rounded-2xl py-4 px-6 text-sm outline-none">
                        </div>
                        <div class="space-y-2">
                            <label class="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Selesai
                                *</label>
                            <input type="date" x-model="tglSelesai" @input="saveLocal()" @input="isDirty = true"
                                required class="w-full bg-gray-50 rounded-2xl py-4 px-6 text-sm outline-none">
                        </div>
                    </div>
                            ...
                            <div @click="selectSubtes(index)"
                                ...
                                <div class="flex justify-between items-start mb-4">
                                    <div class="p-3 rounded-2xl"
                                        ...
                                        <i class="fa-solid"
                                            :class="sub.completed ? 'fa-check-double' : 'fa-book-open'"></i></div>
                                    <span class="text-[10px] font-bold text-gray-400 uppercase"
                                        x-text="sub.completed ? 'Selesai' : 'Draft'"></span>
                                </div>
                                <h3 class="font-bold text-gray-800 text-sm" x-text="sub.name"></h3>
                                <div class="flex items-center justify-between mt-4">
                                    <p class="text-[10px] text-gray-400">Terisi: <span
                                            class="font-bold text-[#4A72D4]" x-text="sub.soalTerisi"></span>/<span
                                            x-text="targetSoal"></span></p>
                    ...
                    <div
                        ...
                        <button type="button" @click="publikasikan()" :disabled="!canPublish"
                            class="px-16 py-4 bg-orange-500 disabled:bg-gray-200 text-white rounded-2xl font-bold shadow-xl transition-all hover:scale-105 active:scale-95">PUBLIKASIKAN
                            TRYOUT</button>
                    </div>
                </div>
                ...
                <div x-show="activeSubtesIndex !== null" x-cloak x-transition>
                    <div class="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
                        <div class="flex items-center gap-4">
                            <button type="button" @click="backToKategori()"
                                class="p-3 bg-white rounded-xl text-gray-400 hover:text-red-500 shadow-sm border border-blue-50 transition-all">
                                <i class="fa-solid fa-arrow-left"></i>
                            </button>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                        ...
                                        <template x-for="opt in ['A','B','C','D','E']">
                                            <div
                                                ...
                                                <span
                                                    ...
                                                    x-text="opt"></span>
                                                <textarea :id="'opt_' + opt.toLowerCase()" placeholder="Isi opsi..."
                        ...
                        <div class="space-y-6">
                            <div
                                class="bg-white p-6 rounded-xl shadow-sm border border-blue-50 text-center sticky top-8">
                                ...
                                <button type="button" @click="selesaikanSubtes()"
                                    :disabled="subtesList[activeSubtesIndex]?.soalTerisi < targetSoal"
                                    :class="subtesList[activeSubtesIndex]?.soalTerisi < targetSoal ? 'bg-gray-300' :
                                        'bg-emerald-500 shadow-lg'"
                                    class="w-full py-4 text-white rounded-2xl font-black text-[10px] uppercase">Selesaikan
                                    Subtes</button>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        function tryoutHandler() {
            return {
                ...
                get canPublish() {
                    return this.namaTryout && this.tglMulai && this.tglSelesai && this.subtesList.every(s => s
                        .completed);
                },
                ...
                unduhTemplate() {
                    ...
                    const header = [
                        ['Pertanyaan', 'Opsi A', 'Opsi B', 'Opsi C', 'Opsi D', 'Opsi E', 'Jawaban Benar', 'Pembahasan',
                            'Materi Teks', 'Link Gambar'
                        ]
                    ];
                    ...
                    reader.onload = (e) => {
                        try {
                            ...
                            workbook.SheetNames.forEach(sheetName => {
                                const sIdx = this.subtesList.findIndex(s => s.name.toLowerCase() === sheetName
                                    .toLowerCase());
                                if (sIdx !== -1) {
                                    foundSheet = true;
                                    ...
                                    if (jsonData.length > 0) {
                                        ...
                                        jsonData.forEach((row, i) => {
                                            if (i < this.targetSoal) {
                                                current.questions[i] = {
                                                    ...
                                                    opsi_d: row['Opsi D'] || '',
                                                    opsi_e: row['Opsi E'] || '',
                                                    jawaban_benar: row['Jawaban Benar']?.toString()
                                                        .toUpperCase() || '',
                                        ...
                                        current.soalTerisi = current.questions.filter(x => x && x.pertanyaan)
                                            .length;
                ...
                loadQuestion() {
                    ...
                    if (data) {
                        ...
                        document.getElementsByName('correct_option').forEach(r => r.checked = (r.value === data
                            .jawaban_benar));
                    } else {
                        this.resetInputForm();
                    }
