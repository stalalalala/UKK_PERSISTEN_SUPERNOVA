        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar p-4 lg:px-8 lg:pb-8">
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mb-8">
                ...
                <div class="bg-white p-6 rounded-xl shadow-sm border border-blue-50 flex items-center gap-5">
                    ...
                    <div>
                        ...
                        <h4 class="text-2xl font-bold text-gray-800" x-text="allTryout.filter(t => t.is_active).length"></h4>
                    </div>
                </div>
                <div class="bg-white p-6 rounded-xl shadow-sm border border-blue-50 flex items-center gap-5 sm:col-span-2 md:col-span-1">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-blue-50 overflow-hidden flex flex-col">
                <div class="p-6 md:p-8 border-b border-gray-50 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h3 class="text-xl font-bold text-gray-800" x-text="activeTab === 'list' ? 'Daftar Tryout' : 'History / Tempat Sampah'"></h3>
                        <p class="text-sm text-gray-400" x-text="activeTab === 'list' ? 'Kelola subtes, soal, dan durasi tryout' : 'Data yang dihapus sementara dapat dipulihkan di sini'"></p>
                    </div>
                    <div class="flex flex-wrap items-center gap-3">
                        <button @click="activeTab = activeTab === 'list' ? 'history' : 'list'; currentPage = 1"
                            ...
                            <i :class="activeTab === 'list' ? 'fa-solid fa-clock-rotate-left' : 'fa-solid fa-list'"></i>
                ...
                <div class="table-responsive custom-scrollbar">
                    <table x-show="activeTab === 'list'" class="w-full text-left border-separate border-spacing-0 min-w-[800px]">
                        <thead class="bg-gray-50/50 sticky top-0 z-10 backdrop-blur-sm">
                            <tr>
                                <th class="px-8 py-4 text-[11px] font-bold text-gray-400 uppercase tracking-widest">Informasi Tryout</th>
                                <th class="px-16 md:px-8 py-4 text-[11px] font-bold text-gray-400 uppercase tracking-widest text-center">Subtes</th>
                                <th class="px-12 md:px-8 py-4 text-[11px] font-bold text-gray-400 uppercase tracking-widest text-center">Total Soal</th>
                                <th class="px-16 md:px-8 py-4 text-[11px] font-bold text-gray-400 uppercase tracking-widest text-center">Durasi</th>
                            ...
                            <template x-for="(to, index) in pagedTryout" :key="to.id">
                                <tr class="hover:bg-blue-50/30 transition-colors group">
                                    ...
                                    <td class="px-8 py-5 text-center">
                                        <span class="bg-indigo-50 text-indigo-600 text-[11px] font-bold px-3 py-1.5 rounded-xl border border-indigo-100" x-text="to.subtes + ' Subtes'"></span>
                                    </td>
                                    ...
                                    <td class="px-8 py-5 text-center">
                                        <form :action="to.toggle_url" method="POST" class="inline">
                                            ...
                                            @method('PATCH')
                                            <button type="submit" 
                                                :class="to.is_active ? 'bg-emerald-100 text-emerald-600 border-emerald-200' : 'bg-red-50 text-red-500 border-red-100'"
                                    ...
                                    <td class="px-8 py-5 text-center">
                                        <div class="flex items-center justify-center gap-2">
                                            ...
                                            <button @click="deleteToHistory(to.id)" class="p-2 text-rose-400 hover:bg-rose-50 rounded-lg transition-all" title="Hapus">
                    ...
                    <table x-show="activeTab === 'history'" class="w-full text-left border-separate border-spacing-0 min-w-[800px]">
                        <thead class="bg-orange-50/50 sticky top-0 z-10 backdrop-blur-sm">
                            <tr>
                                <th class="px-8 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest">Judul Terhapus</th>
                                <th class="px-8 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest text-center">Subtes</th>
                                <th class="px-8 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest text-center">Status Terakhir</th>
                                <th class="px-8 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest text-center">Aksi Pemulihan</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-50">
                                ...
                                <tr class="hover:bg-orange-50/30 transition-colors group">
                                    <td class="px-8 py-5">
                                        <div class="flex flex-col">
                                            ...
                                            <span class="text-[10px] text-rose-400 font-bold uppercase">SAMPAH / HISTORY</span>
                                        </div>
                                    </td>
                                    <td class="px-8 py-5 text-center">
                                        <span class="bg-gray-50 text-gray-600 text-[11px] font-bold px-3 py-1.5 rounded-xl border border-gray-100" x-text="hist.subtes + ' Subtes'"></span>
                                    </td>
                                    <td class="px-8 py-5 text-center">
                                        <span class="bg-gray-100 text-gray-500 px-3 py-1 rounded-full text-[10px] font-black uppercase" x-text="hist.is_active ? 'Aktif' : 'Nonaktif'"></span>
                                    </td>
                                    <td class="px-8 py-5 text-center">
                ...
                <div class="p-6 md:p-8 border-t border-gray-50 flex flex-col sm:flex-row items-center justify-between gap-4 bg-white">
                    <p class="text-[10px] md:text-[11px] text-gray-400 font-bold uppercase tracking-wider">
                        ...
                        <span class="text-gray-700" x-text="activeTab === 'list' ? Math.min(currentPage * itemsPerPage, allTryout.length) : Math.min(currentPage * itemsPerPage, historyTryout.length)"></span>
                        dari <span class="text-gray-700" x-text="activeTab === 'list' ? allTryout.length : historyTryout.length"></span> Data
                    </p>
                        ...
                        <div class="flex gap-2">
                            <template x-for="page in totalPages" :key="page">
                                <button @click="currentPage = page"
                                    :class="currentPage === page ? 'bg-[#4A72D4] text-white shadow-blue-100' : 'border border-gray-100 text-gray-600 hover:bg-gray-50'"
