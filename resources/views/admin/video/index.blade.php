...
<body class="bg-slate-100 font-po overflow-x-hidden">
    ...
    <div x-data="videoApp()" class="flex h-screen overflow-hidden">
        ...
        <aside x-data="{ currentPage: 'video' }"
            ...
            <section class="flex-1 p-4 lg:p-6 xl:p-8 overflow-y-auto">
                ...
                <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-4 md:p-6 overflow-hidden">
                    ...
                    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                        <div class="flex bg-gray-100 p-1 rounded-2xl w-full md:w-auto">
                            <button @click="activeTab = 'active'"
                                :class="activeTab === 'active' ? 'bg-white shadow-sm text-[#4A72D4]' :
                                    'text-gray-500 hover:text-gray-700'"
                                class="px-6 py-2 rounded-xl text-sm font-bold transition-all flex-1 md:flex-none">Daftar
                                Video</button>
                            <button @click="activeTab = 'history'"
                                :class="activeTab === 'history' ? 'bg-white shadow-sm text-[#4A72D4]' :
                                    'text-gray-500 hover:text-gray-700'"
                                ...
                                <tbody class="divide-y divide-gray-50">
                                    @foreach ($videos as $video)
                                        <tr class="hover:bg-slate-50 transition-colors">
                                            ...
                                            <td class="p-4">
                                                <span
                                                    class="bg-blue-50 text-[#4A72D4] px-3 py-1 rounded-full text-[11px] font-bold">{{ $video->subtes }}</span>
                                            </td>
                                            <td class="p-4 font-semibold text-slate-700">{{ $video->judul_video }}
                                            </td>
                                            <td class="p-4">
                                    ...
                                    Swal.fire({
                                        title: 'Hapus Video?',
                                        text: 'Video akan dipindahkan ke History',
                                        icon: 'warning',
                                        width: '340px',
                                        ...
                                        if(result.isConfirmed) $refs['formDelete{{ $video->id }}'].submit() 
                                                ...
                                                <form x-ref="formDelete{{ $video->id }}"
                                                    action="{{ route('admin.videoPembelajaran.destroy', $video->id) }}"
                    ...
                    <div x-show="activeTab === 'history'" x-transition:enter="transition ease-out duration-300"
                        x-transition:enter-start="opacity-0 translate-y-2" style="display: none;">
                                ...
                                <tbody class="divide-y divide-gray-50">
                                    @foreach ($history as $item)
                                        <tr class="hover:bg-gray-50">
                                            ...
                                            <td class="p-4">
                                                <span
                                                    class="bg-blue-50 text-[#4A72D4] px-3 py-1 rounded-full text-[11px] font-bold">{{ $item->subtes }}</span>
                                            </td>
                                            <td class="p-4 font-semibold text-slate-700">{{ $item->judul_video }}</td>
                                            <td class="p-4">
                                                <div class="w-28 h-20 overflow-hidden rounded-lg bg-gray-100">
                                                    <div class="w-full h-full">
                                                        {!! preg_replace(['/width=".*?"/', '/height=".*?"/'], ['width="100%"', 'height="100%"'], $item->iframe) !!}
                                                    </div>
                                            </td>
                                            <td class="p-4 text-center space-x-2">
                                                <button
                                    ...
                                    Swal.fire({
                                         title: 'Pulihkan Video?',
                                            ...
                                            cancelButtonText: 'Batal',
                                        customClass: { popup: 'rounded-3xl shadow-xl', title: 'text-lg font-bold', confirmButton: 'px-5 py-2.5 rounded-xl text-sm',   cancelButton: 'px-5 py-2.5 rounded-xl text-sm bg-gray-100 text-gray-600 hover:bg-gray-200' }
                                    }).then((result) => { if(result.isConfirmed) $refs['formRestore{{ $item->id }}'].submit(); })"
                                    ...
                                    Swal.fire({
                                        ...
                                        cancelButtonText: 'Batal',
                                        customClass: { popup: 'rounded-3xl shadow-xl', title: 'text-lg font-bold', confirmButton: 'px-5 py-2.5 rounded-xl text-sm',   cancelButton: 'px-5 py-2.5 rounded-xl text-sm bg-gray-100 text-gray-600 hover:bg-gray-200' }
                                    }).then((result) => { if(result.isConfirmed) $refs['formForceDelete{{ $item->id }}'].submit(); })"
                                                    class="text-red-500 px-3 py-1.5 rounded-lg text-xs hover:bg-red-600 hover:text-white transition-all shadow-sm">
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                                        viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"
                                                        class="size-6">
                                                ...
                                                <form x-ref="formRestore{{ $item->id }}"
                                                    action="{{ route('admin.videoPembelajaran.restore', $item->id) }}"
                                                    method="POST" class="hidden">@csrf</form>
                                                <form x-ref="formForceDelete{{ $item->id }}"
                                                    action="{{ route('admin.videoPembelajaran.force-delete', $item->id) }}"
        ...
        <div x-show="openModalVideo" x-cloak style="display: none;"
            ...
            <div @click.away="closeModal()" class="bg-white w-full max-w-md rounded-[32px] p-8 shadow-2xl">
                <div class="flex justify-between items-center mb-6">
                    <h3 class="text-xl font-extrabold text-slate-800"
                        x-text="isEditVideo ? 'Ubah Video' : 'Tambah Video'"></h3>
                ...
                <form
                    :action="isEditVideo ? `/admin/videoPembelajaran/${videoData.id}` :
                        '{{ route('admin.videoPembelajaran.store') }}'"
                    method="POST">
                    @csrf
                    ...
                    <div class="space-y-1 mb-3">
                        <label class="text-[10px] font-bold text-slate-400 uppercase ml-1 tracking-widest">Kategori
                            Subtes</label>
                    ...
                    <div class="space-y-1 mb-3">
                        <label class="text-[10px] font-bold text-slate-400 uppercase ml-1 tracking-widest">Judul
                            Video</label>
                    ...
                    <div class="space-y-1 mb-6">
                        <label class="text-[10px] font-bold text-slate-400 uppercase ml-1 tracking-widest">Iframe
                            Video</label>
                        <textarea name="iframe" x-model="videoData.iframe"
                            class="w-full bg-[#F3F6FF] border-none rounded-2xl p-3 text-sm focus:ring-2 focus:ring-blue-400 outline-none"
                            placeholder='<iframe ...></iframe>' rows="4" required oninput="validateIframe(this)"></textarea>
                        <p class="text-[10px] text-red-500 mt-1 hidden" id="iframe-error">Input harus berupa tag
                            &lt;iframe&gt; yang valid.</p>
                    </div>

                    <div class="flex gap-3">
                        <button type="button" @click="closeModal()"
                            class="flex-1 bg-slate-50 text-slate-400 font-bold py-3.5 rounded-2xl hover:bg-slate-100 transition-all">Batal</button>
                        <button type="submit"
                            class="flex-1 bg-[#4A72D4] text-white font-bold py-3.5 rounded-2xl hover:bg-blue-600 shadow-lg shadow-blue-100 transition-all active:scale-95">Simpan</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div x-show="openImportModalVideo" x-cloak class="fixed inset-0 z-[100] overflow-y-auto">
            ...
            <div class="fixed inset-0 bg-black/60 backdrop-blur-sm transition-opacity" @click="closeImportModal()">
            </div>

            <div class="relative min-h-screen flex items-center justify-center p-4">
                <div class="relative bg-white w-full max-w-lg rounded-[35px] shadow-2xl p-8 transform transition-all"
                    x-show="openImportModalVideo" x-transition:enter="transition ease-out duration-300"
                    ...
                    <div
                        ...
                        <input type="file" class="hidden" x-ref="videoExcelInput"
                            @change="handleFileUpload($event)" accept=".xlsx,.xls">
        ...
        function videoApp() {
            return {
                ...
                unduhTemplate() {
                    ...
                    // 1. Sheet Utama
                    const header = [
                        ['subtes', 'judul_video', 'iframe']
                    ];
                    const dummyData = [
                        ['Penalaran Umum', 'Contoh Judul Video', '<iframe src="..."></iframe>']
                    ];
