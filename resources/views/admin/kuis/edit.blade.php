...
<head>
    ...
    <title>Edit Kuis Fundamental - Admin | PERSISTEN</title>
    ...
    function editKuisData() {
        return {
            ...
            simpanSoal() {
                ...
                // VALIDASI
                if (!this.currentQuestion.pertanyaan || this.currentQuestion.pertanyaan.trim() === '') {
                ...
                // VALIDASI OPSI A-E
                const opsi = [
                    this.currentQuestion.opsi_a,
                    this.currentQuestion.opsi_b,
                    this.currentQuestion.opsi_c,
                    this.currentQuestion.opsi_d,
                    this.currentQuestion.opsi_e,
                ];

                const kosongIndex = opsi.findIndex(opt => !opt || opt.trim() === '');

                if (kosongIndex !== -1) {
                    alert(`Jawaban ${['A','B','C','D','E'][kosongIndex]} wajib diisi!`);
                    return;
                }

                if (!this.currentQuestion.jawaban_benar) {
                    alert("Pilih jawaban benar!");
                    return;
                }

                // TANDAI SUDAH DISIMPAN
                this.currentQuestion.status = "original";

                // PINDAH KE SOAL SELANJUTNYA
                if (this.activeQuestionIndex < this.questions.length - 1) {
                    this.activeQuestionIndex++;
                    this.currentQuestion = this.questions[this.activeQuestionIndex];
                }
...
<body class="bg-[#E9EFFF] h-screen overflow-hidden text-[#2D3B61]" x-data="{ ...PageGuard(), ...editKuisData() }" x-init="init()">
                            ...
                            <div
                                ...
                                <div class="flex items-center gap-2 w-full md:w-auto order-1 md:order-2">
                                    ...
                                    <button type="button" @click="simpanSoal()"
                                        class="bg-[#4A72D4] hover:bg-blue-600 text-white px-8 py-3 rounded-xl font-bold text-sm">
                                        Simpan Soal
                                    </button>
