...
<body class="bg-blue-50 flex items-center justify-center min-h-screen p-2 sm:p-4" 
    ...
    <form action="{{ route('minatbakat.store') }}" method="POST" x-ref="minatBakatForm" class="w-full flex justify-center">
        @csrf
        ...
        <div class="bg-white rounded-3xl shadow-xl w-full max-w-[1440px] mx-10 overflow-hidden border border-blue-100 flex flex-col">
            ...
            <div class="p-4 sm:p-6 flex justify-between items-center">
                ...
                <div class="flex flex-row items-center gap-2">
                    <div class="bg-white border border-gray-200 px-3 py-2 rounded-full text-xs sm:text-sm font-bold text-gray-500 shadow-sm whitespace-nowrap">
                        Soal <span x-text="step"></span> dari {{ $soals->count() }}
                    </div>
                    <a href="/">
                        <button type="button" class="px-8 py-2 rounded-full font-bold text-gray-400 bg-gray-200 hover:bg-gray-300 transition">Batal</button>
                    </a>
                </div>
            </div>
            ...
            <div class="px-6 sm:px-12 mb-4">
                <div class="h-2 w-full bg-blue-100 rounded-full overflow-hidden">
                    <div class="h-full bg-blue-500 transition-all duration-700 ease-out" :style="`width: ${(step / totalSteps) * 100}%`"></div>
                </div>
            </div>

            <div class="mx-4 sm:mx-8 mb-8 bg-slate-50 rounded-[2rem] sm:rounded-[2.5rem] p-6 sm:p-10 relative border border-blue-50">
                <div class="absolute -top-5 left-1/2 -translate-x-1/2 w-10 h-10 bg-blue-500 text-white rounded-full flex items-center justify-center font-bold shadow-lg ring-4 ring-white" x-text="step"></div>

                @foreach($soals as $index => $s)
                <div x-show="step === {{ $index + 1 }}" x-cloak data-step="{{ $index + 1 }}" data-soal-id="{{ $s->id }}">
                    ...
                    <div class="grid grid-cols-5 gap-2 sm:gap-4 items-end w-full">
                        ...
                        <div class="flex flex-col items-center">
                            <button type="button" @click="selected = 5"
                                ...
                                :class="selected === 5 ? 'ring-2 sm:ring-4 ring-blue-200 shadow-lg scale-105' : 'opacity-70 hover:opacity-100'">
                        ...
                        <div class="flex flex-col items-center">
                            <button type="button" @click="selected = 4"
                                ...
                                :class="selected === 4 ? 'ring-2 sm:ring-4 ring-blue-100 shadow-lg scale-105' : 'opacity-70 hover:opacity-100'">
                        ...
                        <div class="flex flex-col items-center">
                            <button type="button" @click="selected = 2"
                                ...
                                :class="selected === 2 ? 'ring-2 sm:ring-4 ring-red-100 shadow-lg scale-105' : 'opacity-70 hover:opacity-100'">
                        ...
                        <div class="flex flex-col items-center">
                            <button type="button" @click="selected = 1"
                                ...
                                :class="selected === 1 ? 'ring-2 sm:ring-4 ring-red-200 shadow-lg scale-105' : 'opacity-70 hover:opacity-100'">
