...
<body class="bg-blue-50 flex items-center justify-center min-h-screen p-2 sm:p-4" x-data="{
    ...
    selected: null,
    jawaban: {},
    // Menyimpan ID soal saat ini berdasarkan urutan loop
    currentSoalId: null,
    ...
    updateCurrentId() {
        // Mengambil ID soal dari atribut data pada elemen yang sedang aktif
        this.$nextTick(() => {
            const el = document.querySelector(`[data-step='${this.step}']`);
            if (el) this.currentSoalId = el.getAttribute('data-soal-id');
    ...
    nextStep() {
        if (this.selected !== null) {
            // Simpan jawaban dengan KEY berupa ID Soal Database
    ...
    <form action="{{ route('minatbakat.store') }}" method="POST" x-ref="minatBakatForm"
        ...
        <div
            ...
            <div class="p-4 sm:p-6 flex justify-between items-center">
                ...
                <div class="flex flex-row items-center gap-2">
                    <div
                        ...
                        Soal <span x-text="step"></span> dari {{ $soals->count() }}
                    </div>
                    <a href="/">
                        <button type="button"
            ...
            <div
                class="mx-4 sm:mx-8 mb-8 bg-slate-50 rounded-[2rem] sm:rounded-[2.5rem] p-6 sm:p-10 relative border border-blue-50">
                ...
                @foreach ($soals as $index => $s)
                    <div x-show="step === {{ $index + 1 }}" x-cloak data-step="{{ $index + 1 }}"
                        data-soal-id="{{ $s->id }}">
                        <p
                            class="text-center text-slate-600 text-base sm:text-xl leading-relaxed font-semibold mb-8 sm:mb-12">
                            {{ $s->text }}
                        </p>
                    </div>
                @endforeach
                ...
                <div class="max-w-3xl mx-auto flex flex-col items-center">
                    <div class="grid grid-cols-5 gap-2 sm:gap-4 items-end w-full">
                        ...
                        <div class="flex flex-col items-center">
                            <button type="button" @click="selected = 5"
                                ...
                                :class="selected === 5 ? 'ring-2 sm:ring-4 ring-blue-200 shadow-lg scale-105' :
                            ...
                            <div class="py-2 sm:py-4">
                                <svg xmlns="http://www.w3.org/2000/svg"
                                    class="h-6 w-6 sm:h-10 sm:w-10 text-blue-500 transition-all duration-300"
                                    :class="selected === 5 ? 'scale-125 opacity-100' : 'opacity-40'" fill="none"
                        ...
                        <div class="flex flex-col items-center">
                            <button type="button" @click="selected = 4"
                                ...
                                :class="selected === 4 ? 'ring-2 sm:ring-4 ring-blue-100 shadow-lg scale-105' :
                                    'opacity-70 hover:opacity-100'">
                                <span
                                    class="text-white btn-text font-bold uppercase text-center leading-none">Setuju</span>
                            </button>
                            <div class="py-2 sm:py-4">
                                <svg xmlns="http://www.w3.org/2000/svg"
                                    class="h-6 w-6 sm:h-10 sm:w-10 text-blue-400 transition-all duration-300"
                                    :class="selected === 4 ? 'scale-125 opacity-100' : 'opacity-40'" fill="none"
                                    viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                                        d="M14.828 14.828a4 4 0 01-5.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                        ...
                        <div class="flex flex-col items-center">
                            <button type="button" @click="selected = 3"
                                ...
                                :class="selected === 3 ? 'ring-2 sm:ring-4 ring-gray-200 shadow-lg scale-105' :
                                    'opacity-70 hover:opacity-100'">
                                <span
                                    class="text-white btn-text font-bold uppercase text-center leading-none">Netral</span>
                            </button>
                            <div class="py-2 sm:py-4">
                                <svg xmlns="http://www.w3.org/2000/svg"
                                    class="h-6 w-6 sm:h-10 sm:w-10 text-blue-300 transition-all duration-300"
                                    :class="selected === 3 ? 'scale-125 opacity-100' : 'opacity-40'" fill="none"
                                    viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                        ...
                        <div class="flex flex-col items-center">
                            <button type="button" @click="selected = 2"
                                ...
                                :class="selected === 2 ? 'ring-2 sm:ring-4 ring-red-100 shadow-lg scale-105' :
                                    'opacity-70 hover:opacity-100'">
                            ...
                            <div class="py-2 sm:py-4">
                                <svg xmlns="http://www.w3.org/2000/svg"
                                    class="h-6 w-6 sm:h-10 sm:w-10 text-blue-400 transition-all duration-300"
                                    :class="selected === 2 ? 'scale-125 opacity-100' : 'opacity-40'" fill="none"
                                    viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                                        d="M9.172 15.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col items-center">
                            <button type="button" @click="selected = 1"
                                ...
                                :class="selected === 1 ? 'ring-2 sm:ring-4 ring-red-200 shadow-lg scale-105' :
                                    'opacity-70 hover:opacity-100'">
                                <span class="text-white btn-text font-bold uppercase text-center leading-none">Sangat
                                    Tidak Setuju</span>
                            </button>
                            <div class="py-2 sm:py-4">
                                <svg xmlns="http://www.w3.org/2000/svg"
                                    class="h-6 w-6 sm:h-10 sm:w-10 text-blue-500 transition-all duration-300"
                                    :class="selected === 1 ? 'scale-125 opacity-100' : 'opacity-40'" fill="none"
                                    viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                                        d="M9.172 15.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                    ...
                    <div
                        class="h-2 w-full rounded-full bg-gradient-to-r from-blue-400 via-gray-300 to-red-400 opacity-40 mt-[-4px]">
                    </div>
                </div>
            </div>
            ...
            <div class="p-4 sm:p-6 flex justify-between items-center bg-gray-50/50">
                ...
                <button type="button" @click="nextStep()" :disabled="selected === null"
                    ...
                    <span x-text="step === totalSteps ? 'Selesai & Lihat Hasil' : 'Selanjutnya'"></span>
                </button>
            </div>
        </div>
    </form>
