...
<head>
    ...
    <title>Jeda Try Out | PERSISTEN</title>
...
<body class="bg-white font-po overflow-x-hidden" x-data="{
    timeLeft: 0,
    timerText: '',
    canContinue: false,
    showExitModal: false,
            ...
            <div
                ...
                <div class="mb-10 text-center">
                    ...
                    <p class="text-[#2E3B66] text-sm md:text-base max-w-2xl mx-auto leading-relaxed"
                        x-show="!canContinue">Istirahatlah sejenak sebelum memulai subtes berikutnya.</p>
                    <p class="text-green-500 text-sm md:text-base font-bold max-w-2xl mx-auto leading-relaxed"
                        x-show="canContinue" x-cloak>Waktu istirahat selesai! Silahkan lanjut ke subtes berikutnya.</p>
                </div>
                    ...
                    <div class="p-6 grid grid-cols-1 md:grid-cols-3 gap-y-4 gap-x-6">
                        @foreach ($categories as $cat)
                            <div class="flex items-center gap-3">
                                <div
                                    class="w-6 h-6 rounded-full flex items-center justify-center {{ $cat->id < $next_category_id ? 'bg-green-500 text-white text-[10px]' : 'bg-white border-2 border-gray-200' }}">
                ...
                <div class="flex flex-col md:flex-row justify-center items-center gap-4">
                    ...
                    <div class="w-full md:w-auto">
                        <a :href="canContinue ? '{{ route('tryout.soal', [$tryout->id, $next_category_id]) }}' : '#'"
                            class="block w-full" @click="if(!canContinue) $event.preventDefault()">
                            <button
                                :class="canContinue ? 'bg-[#6EB4FF] shadow-lg active:scale-95' :
    ...
    <div x-show="showExitModal" class="fixed inset-0 z-[99] flex items-center justify-center p-4" x-cloak>
        ...
        <div class="bg-white rounded-[2rem] p-8 max-w-sm w-full relative z-10 text-center shadow-2xl">
            ...
            <p class="text-gray-500 text-sm mb-8">Data pengerjaan Anda akan hilang jika keluar sekarang. Yakin ingin
                keluar?</p>
            <div class="flex gap-4">
                <button @click="showExitModal = false"
                    class="flex-1 py-3 rounded-xl font-bold bg-gray-100 text-gray-500">Batal</button>
                <a href="{{ route('tryout.index') }}" @click="clearSessionData()"
