...
<head>
    ...
    <title>Pengerjaan {{ $tryout->nama_tryout }} | PERSISTEN</title>
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 px-2">
            ...
            <div class="flex items-center justify-between md:justify-end gap-3 sm:gap-4">
                ...
                <button @click="showExitModal = true"
                    class="bg-[#3B82F6] px-6 py-3 rounded-3xl text-white font-bold text-sm">Keluar Ujian</button>
            </div>
        </div>
            ...
            <div class="flex-1 p-5 sm:p-10 md:p-14 lg:p-16 border-b lg:border-b-0 lg:border-r border-gray-100">
                @foreach ($soals as $index => $soal)
                    <div x-show="soalAktif === {{ $index + 1 }}" x-cloak>
                        <p class="text-xs md:text-sm font-semibold text-[#2E3B66] mb-4">Subtes:
                            {{ $currentCategory->nama_kategori }}</p>

                        @if ($soal->materi_teks)
                            <div
                                class="mb-6 p-4 bg-slate-50 border-l-4 border-[#3B82F6] rounded-r-xl text-gray-700 text-sm md:text-base leading-relaxed italic">
                                {!! $soal->materi_teks !!}
                            </div>
                        @endif
                        ...
                        @if ($soal->image_url)
                            <div class="mb-8 flex justify-center">
                                <div
                                    class="max-w-full overflow-hidden rounded-2xl border-2 border-gray-100 shadow-sm bg-white">
                ...
                <div class="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-7 gap-4 max-w-md">
                    @foreach ($soals as $index => $soal)
                        <button @click="soalAktif = {{ $index + 1 }}"
                            ...
                            :class="soalAktif === {{ $index + 1 }} ? 'bg-[#4FAAFD] border-[#4FAAFD] text-white' : (
                                jawabanTerpilih['{{ $soal->id }}'] ?
                                'border-[#4FAAFD] text-[#4FAAFD] bg-blue-50' :
                                'border-[#4FAAFD] text-[#4FAAFD] bg-white')">
                            <span>{{ $index + 1 }}</span>
                            <template x-if="jawabanTerpilih['{{ $soal->id }}']">
                                <div
                                    class="absolute -top-2 -right-2 w-6 h-6 bg-white text-[#4FAAFD] text-xs rounded-full flex items-center justify-center border-2 border-[#4FAAFD] font-bold z-10">
                ...
                <div class="flex items-center gap-3 mt-10 md:mt-14">
                    <button @click="if(soalAktif > 1) soalAktif--" :disabled="soalAktif === 1"
                        ...
                        :class="soalAktif > 1 ? 'border-[#3B82F6] text-[#3B82F6] bg-blue-50' :
                            'border-gray-200 text-gray-400 opacity-50'">
                        Kembali
                    </button>

                    <button
                        @click="if(soalAktif < totalSoal) { 
                                soalAktif++ 
                            } else { 
                                if(adaSoalKosong()) {
                    ...
                    <div x-show="soalAktif === {{ $index + 1 }}" class="space-y-4">
                        @foreach (['a', 'b', 'c', 'd', 'e'] as $opt)
                            ...
                            <div @click="jawabanTerpilih['{{ $soal->id }}'] = '{{ strtoupper($opt) }}'"
                                :class="jawabanTerpilih['{{ $soal->id }}'] === '{{ strtoupper($opt) }}' ?
                                    'border-[#3378FF] ring-1 ring-[#3378FF]' : 'border-transparent'"
                                ...
                                <div class="w-9 h-9 rounded-full flex items-center justify-center font-bold mr-4 shrink-0"
                                    :class="jawabanTerpilih['{{ $soal->id }}'] === '{{ strtoupper($opt) }}' ?
                                        'bg-[#3378FF] text-white' : 'bg-white text-[#3378FF]'">
                                    {{ strtoupper($opt) }}
                                </div>
                                <span
                                    class="text-sm md:text-base font-medium text-[#2E3B66]">{{ $soal->$kolom }}</span>
                            </div>
                        @endforeach
                    </div>
                @endforeach
        ...
        <div x-show="showExitModal" class="fixed inset-0 z-[99] flex items-center justify-center p-4" x-transition
            ...
            <div class="bg-white rounded-[2rem] p-8 max-w-sm w-full relative z-10 text-center shadow-2xl">
                ...
                <div class="flex gap-4">
                    ...
                    <a href="{{ route('tryout.index') }}" @click="clearData()"
                        class="flex-1 py-3 rounded-xl font-bold bg-red-500 text-white text-center">Ya, Keluar</a>
                </div>
            </div>
        </div>

        <div x-show="showConfirmNextModal" class="fixed inset-0 z-[99] flex items-center justify-center p-4"
            ...
            <div class="bg-white rounded-[2rem] p-8 max-w-sm w-full relative z-10 text-center shadow-2xl">
                ...
                <p class="text-gray-500 text-sm mb-8">Masih ada soal yang belum dijawab. Yakin ingin melanjutkan ke
                    subtes berikutnya?</p>
                <div class="flex flex-col gap-3">
                    <button @click="pindahHalaman()"
                        class="w-full py-3 rounded-xl font-bold bg-[#3B82F6] text-white">Ya, Lanjutkan</button>
                    <button @click="showConfirmNextModal = false"
