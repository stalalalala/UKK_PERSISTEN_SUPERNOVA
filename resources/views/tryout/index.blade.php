...
<body class="bg-white min-h-screen" x-data="{
    ...
    <main class="max-w-[1440px] mx-auto px-4 md:px-10 py-8">
        <section x-data="{ openUniv: false, openJurusan: false }"
            ...
            <div class="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div @click="openUniv = !openUniv; openJurusan = false"
                        ...
                        <div class="flex justify-between items-center font-bold text-lg text-[#2E3B66]">
                            ...
                            <i class="fa-solid fa-chevron-down text-gray-400 text-sm transition-transform"
                                :class="openUniv ? 'rotate-180' : ''"></i>
                        </div>
                    </div>
                    <div x-show="openUniv" @click.away="openUniv = false" x-cloak
                        class="absolute z-50 w-full mt-2 bg-white border border-gray-100 rounded-2xl shadow-xl max-h-64 overflow-y-auto custom-scrollbar p-2">
                        <template x-for="univ in allUnivs" :key="univ.id">
                            <div @click="univSelectedId = univ.id; univSelectedName = univ.nama_univ; openUniv = false; jurusanSelectedName = 'Pilih Jurusan'; jurusanSelectedId = ''"
                                class="p-4 hover:bg-blue-50 rounded-xl cursor-pointer font-bold text-[#2E3B66] transition-colors"
                                x-text="univ.nama_univ"></div>
                        </template>
                    </div>
                </div>
                ...
                <div class="relative">
                    <div @click="if(univSelectedId) openJurusan = !openJurusan; openUniv = false"
                        :class="!univSelectedId ? 'opacity-50 cursor-not-allowed bg-gray-50' :
        ...
        <section class="border-2 border-gray-200 rounded-[3rem] p-8 md:p-10 mb-20">
            <div class="mb-10">
                ...
                <p class="text-gray-500 font-medium text-sm">Pilih universitas terlebih dahulu untuk membuka sesi
                    tryout.</p>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                <template x-for="to in paginatedTryouts" :key="to.id">
                    <a :href="to.sudah_dikerjakan ? to.url_hasil : (to.is_open ? to.url_mengerjakan : '#')"
                        ...
                        <div :class="to.sudah_dikerjakan ? 'bg-emerald-50 border-emerald-100 hover:shadow-lg' : (to.is_open ?
                            'bg-blue-50 border-blue-100 hover:shadow-lg' :
                            'bg-gray-100 opacity-60 border-gray-200 cursor-not-allowed grayscale')"
                            class="rounded-[2rem] p-6 flex flex-col items-center relative transition-all group border h-full">

                            <template x-if="to.is_locked">
                                <div
                                    class="absolute -top-3 bg-red-500 text-white text-[10px] px-4 py-1 rounded-full font-black uppercase tracking-widest shadow-md z-20">
                                    Terkunci</div>
                            </template>

                            <div class="w-full flex justify-between items-start mb-2">
                                <span
                                    :class="to.is_open ? 'text-blue-400' : (to.sudah_dikerjakan ? 'text-emerald-400' :
                                        'text-gray-400')"
                                    class="font-bold pt-1 text-lg uppercase tracking-tighter">Try Out</span>
                                <span
                                    :class="to.is_open ? 'bg-blue-600' : (to.sudah_dikerjakan ? 'bg-emerald-600' :
                                        'bg-gray-400')"
                                    class="text-white text-lg px-4 py-1 rounded-full font-bold">UTBK</span>
                            </div>

                            <div :class="to.is_open ? 'text-blue-500 group-hover:scale-110' : (to.sudah_dikerjakan ?
                                'text-emerald-500' : 'text-gray-300')"
                                class="text-[100px] font-black leading-none mt-6 mb-1 transition-transform"
                                x-text="to.nomor"></div>

                            <div class="w-full mb-6 text-center">
                                <div :class="to.is_open ? 'text-[#2E3B66]' : (to.sudah_dikerjakan ? 'text-[#2E3B66]' :
                                    'text-gray-400')"
                            ...
                            <div class="bg-white px-4 py-1.5 rounded-full flex items-center gap-2 text-sm font-medium shadow-sm w-full justify-center mt-auto"
                                :class="to.is_open ? 'text-blue-500' : (to.sudah_dikerjakan ? 'text-emerald-500' :
                                    'text-gray-400')">

                                <template x-if="to.is_open"><i class="fa-solid fa-clock text-[10px]"></i></template>
                                <template x-if="to.sudah_dikerjakan"><i
                                        class="fa-solid fa-circle-check text-[10px]"></i></template>
                                <template x-if="!to.is_open && !to.sudah_dikerjakan"><i
                                        class="fa-solid fa-lock text-[10px]"></i></template>
            ...
            <div class="flex justify-center items-center gap-3 mt-16" x-show="totalPages > 1">
                <button @click="if(currentPage > 1) currentPage--"
                    class="w-10 h-10 rounded-full border border-gray-200 text-gray-400 hover:bg-blue-500 hover:text-white transition-colors"><i
                        class="fa-solid fa-chevron-left text-xs"></i></button>
                <template x-for="page in totalPages" :key="page">
                    <button @click="currentPage = page"
                        :class="currentPage === page ? 'bg-blue-500 text-white shadow-lg' : 'text-blue-500 hover:bg-blue-50'"
                        class="w-10 h-10 rounded-full font-bold transition-all" x-text="page"></button>
                </template>
