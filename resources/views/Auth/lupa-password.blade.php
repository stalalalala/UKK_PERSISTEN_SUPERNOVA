...
<div class="bg-white p-6 sm:p-8 lg:p-10 rounded-3xl shadow-xl w-full max-w-sm sm:max-w-md lg:max-w-lg xl:max-w-xl">
    ...
    @if (session('status'))
        <div class="bg-green-100 text-green-600 p-4 rounded-xl mb-4 text-sm sm:text-base border border-green-200 flex items-center">
    ...
    <div x-data="{ 
        ...
        <form method="POST" action="{{ route('password.email') }}" class="space-y-4" @submit="startCountdown(false)">
            @csrf
            ...
            <button type="submit"
                :disabled="waiting"
                :class="waiting ? 'bg-gray-400 cursor-not-allowed opacity-80' : 'bg-blue-500 hover:bg-blue-600 shadow-lg shadow-blue-200'"
