...
<head>
    ...
    <style>
        @keyframes slime-auto-only-left {
            0%, 15% { transform: rotate(0deg) scale(1, 1); }
            25%, 40% { transform: rotate(-10deg) scale(1.02, 0.98); }
            45% { transform: rotate(0deg) scale(1, 1); }
            50% { transform: scale(1.15, 0.85); }
            55% { transform: scale(0.95, 1.05); }
            60% { transform: scale(1, 1); }
            65%, 100% { transform: rotate(0deg) scale(1, 1); }
        }
        ...
        .slime-shadow { filter: drop-shadow(0 15px 30px rgba(0, 0, 0, 0.3)); }
...
<body class="font-poppins bg-white min-h-screen flex flex-col relative">
    ...
    <div class="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 lg:left-auto lg:right-[-15vw] lg:translate-x-0 h-[90vh] md:h-[110vh] lg:h-[130vh] aspect-square bg-[#4A69BD] rounded-full z-0 flex items-center justify-center lg:justify-start opacity-90 lg:opacity-100 transition-all duration-700 ease-in-out">
        <div class="slime-auto-left w-[300px] md:w-[450px] lg:w-[550px] lg:ml-10 mb-10 relative z-10">
            <object data="{{ asset('img/pet(tanpa_animasi).svg') }}" type="image/svg+xml" class="pointer-events-none select-none w-full h-auto slime-shadow"></object>
        </div>
    </div>
                ...
                <div class="lg:col-span-6 xl:col-span-5 flex justify-center lg:justify-start">
                    <div class="bg-white/10 lg:bg-[#E8F1FF]/90 backdrop-blur-[2px] lg:backdrop-blur-xl w-full max-w-[450px] md:max-w-[480px] rounded-[40px] md:rounded-[50px] p-8 md:p-12 shadow-2xl border-4 border-white/40 lg:border-white/60 transition-all">
                        ...
                        <form action="{{ route('login') }}" method="POST" class="space-y-5">
                            ...
                            <div x-data="{ showPassword: false }" class="space-y-2 relative">
                                ...
                                <input :type="showPassword ? 'text' : 'password'" name="password" placeholder="xxxxxxxx" required
